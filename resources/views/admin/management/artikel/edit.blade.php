    ...
    <div class="col-sm-12 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          ...
          <form class="forms-sample" method="POST" action="{{url('admin/artikel/update')}}" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{$row->id}}">
            <div class="form-group">

              <label for="title">{{Helper::uc('title')}}</label>
              <input type="text" class="form-control" id="title" name="title" placeholder="title" value="{{$row->title}}" required>
            </div>
              ...
              <div class="form-group">
                ...
                <select class="form-control" name="artikel_category_id" id="artikel_category_id" required>
                    <option value="{{$row->artikel_category_id}}">{{$row->category}}</option>
